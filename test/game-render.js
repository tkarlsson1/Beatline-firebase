...
function renderTimeline() {
  ...
  if (displayTeam.pendingCard) {
    addPreviewCardFromFirebase(displayTeam.pendingCard, currentTeamId === teamId);
  }
}
...
function createCardElement(card) {
  ...
  if (card.position === 0) {
    cardDiv.classList.add('starting-card');
  }
...
function addPreviewCardFromFirebase(pendingCard, isMyCard) {
  ...
  `;
  previewCard.style.border = '3px solid #4CAF50';
